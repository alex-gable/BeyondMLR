...
```{r, echo=FALSE}
current_packages <- dplyr::mutate(
  unique(dplyr::select(
    renv::dependencies(errors = "ignored"), Package)),
    PackgeVersion = purrr::map_chr(Package, ~ paste0(packageVersion(.x)))) 

kable_styling(kable(current_packages,
                    booktabs = TRUE, 
                    linesep = "",
                    longtable = TRUE,
                    col.names = c("R package", "Version used")),
